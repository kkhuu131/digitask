...
const AdminTournamentTeamsPage = () => {
  ...
  const [teams, setTeams] = useState<TournamentTeamTemplate[]>(() =>
    TOURNAMENT_TEAM_POOL.map((t) => ({
      ...t,
      digimon: [...t.digimon] as TournamentTeamTemplate['digimon'],
    }))
  );
  ...
  const [slots, setSlots] = useState<[number | null, number | null, number | null]>([
    null,
    null,
    null,
  ]);
  ...
  const filtered = allDigimon.filter(
    (d) =>
  ...
  const saveTeam = () => {
    if (!teamName.trim()) {
      addNotification({ message: 'Team name is required', type: 'error' });
      return;
    }
    if (slots.some((s) => s === null)) {
      addNotification({ message: 'All 3 Digimon slots must be filled', type: 'error' });
      return;
    }
    const id =
      editingId ??
  ...
  const editTeam = (team: TournamentTeamTemplate) => {
    ...
    setSlots(
      team.digimon.map((d) => d.digimon_id) as [number | null, number | null, number | null]
    );
    setActiveSlot(null);
                ...
                {filtered.map((d) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-[10px] font-medium text-gray-800 dark:text-gray-200 text-center mt-1 leading-tight">
                        {d.name}
                      </p>
                      <span
                        className={`text-[9px] px-1.5 py-0.5 rounded-full mt-0.5 font-medium ${STAGE_COLORS[d.stage] ?? 'bg-gray-100 text-gray-600'}`}
                      >
                        {d.stage}
                      </span>
                    </button>
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          <div className="bg-white dark:bg-dark-300 rounded-xl border border-gray-200 dark:border-dark-100 shadow-sm p-4">
                          ...
                          <div className="flex items-center gap-1">
                            <div className="w-10 h-10">
                              <DigimonSprite
                                ...
                                fallbackSpriteUrl={species.sprite_url}
                                size="xs"
                                showHappinessAnimations={false}
                              />
                            </div>
                            <TypeAttributeIcon
                              type={species.type as DigimonType}
                              attribute={species.attribute as DigimonAttribute}
                              size="xs"
                              showTooltip
                            />
                          </div>
                          <p className="text-[10px] font-medium text-gray-700 dark:text-gray-200 mt-1 text-center leading-tight">
                            {species.name}
                          </p>
                          <span
                            className={`text-[9px] px-1.5 py-0.5 rounded-full mt-0.5 ${STAGE_COLORS[species.stage] ?? ''}`}
                          >
                            {species.stage}
                          </span>
                          <button
            ...
            <div className="flex items-center gap-2">
              ...
              {editingId && (
                <button onClick={resetBuilder} className={btnSecondary}>
                  Cancel
                </button>
              )}
            </div>
              ...
              <div className="divide-y divide-gray-100 dark:divide-dark-100">
                {sortedTeams.map((team) => {
                  ...
                  return (
                    <div
                      key={team.id}
                      className={`flex items-center gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-dark-400 transition-colors ${editingId === team.id ? 'bg-indigo-50 dark:bg-accent-900/10' : ''}`}
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`text-[9px] px-1.5 py-0.5 rounded-full font-semibold ${STAGE_COLORS[label] ?? 'bg-gray-100 text-gray-600'}`}
                          >
                        ...
                        {team.digimon.map((d, i) => {
                          ...
                          return (
                            <div key={i} className="flex flex-col items-center">
                              ...
                              {species && (
                                <span
                                  className={`text-[8px] px-1 rounded-full leading-tight ${STAGE_COLORS[species.stage] ?? ''}`}
                                >
                                  {species.stage}
                                </span>
                              )}
          ...
          <div className="bg-white dark:bg-dark-300 rounded-xl border border-gray-200 dark:border-dark-100 shadow-xl w-full max-w-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-dark-100">
              ...
              <button
                ...
                aria-label="Close"
                className="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-dark-200 transition-colors cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
