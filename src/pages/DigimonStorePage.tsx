...
const DigimonStorePage: React.FC = () => {
  ...
  const discoverRandomDigimon = async (userId: string) => {
    try {
      ...
      const { error: insertError } = await supabase.from('user_discovered_digimon').insert({
        user_id: userId,
        ...
        if (item.id === 'brave_point' && userDigimon) {
          try {
            ...
            const { error } = await supabase
              ...
              .update({
                experience_points:
                  (userDigimon.experience_points || 0) + Number(item.effect!.value),
                last_updated_at: new Date().toISOString(),
              })
              ...
              <div className="flex items-center gap-4 mb-6 p-4 bg-gray-50 dark:bg-dark-200 rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Level {selectedDigimonForStatReset.current_level}
                  </p>
                </div>
              </div>
      ...
      {showXAntibodyModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-dark-300 rounded-xl shadow-xl max-w-md w-full max-h-[80vh] overflow-hidden flex flex-col">
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                Choose a Digimon to grant the X-Antibody. This will allow it to transform into its
                X-Antibody form at will (if available).
              </p>
