...
const ProfilePage = () => {
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-3">
            {[1,2,3,4].map(i => <div key={i} className="h-20 bg-gray-100 dark:bg-dark-200 rounded-xl animate-pulse" />)}
          </div>
  ...
  const winRate = profileData.battles_completed > 0
    ? Math.round((profileData.battles_won / profileData.battles_completed) * 100)
    : 0;

  const statCards = [
    { label: 'Victories', value: profileData.battles_won, color: 'text-purple-400', bg: 'bg-purple-400/10 border-purple-400/20' },
    { label: 'Win Rate', value: `${winRate}%`, color: 'text-green-400', bg: 'bg-green-400/10 border-green-400/20' },
    { label: 'Streak', value: `${profileData.current_streak}d`, color: 'text-amber-400', bg: 'bg-amber-400/10 border-amber-400/20' },
    { label: 'DigiDex', value: `${discoveryPercentage()}%`, color: 'text-blue-400', bg: 'bg-blue-400/10 border-blue-400/20' },
  ];
    ...
    <div className="max-w-4xl mx-auto px-2 sm:px-4 py-4 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
          {statCards.map(card => (
            <div key={card.label} className={`rounded-xl border p-3 text-center ${card.bg}`}>
              <div className={`font-body text-xs font-semibold uppercase tracking-wide mb-1 ${card.color}`}>{card.label}</div>
            ...
            <div className="flex-1 min-w-0 w-full">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                {favoriteStatEntries.map(({ key, val }) => (
                  <div key={key} className="bg-gray-50 dark:bg-dark-200 rounded-lg py-2 text-center border border-gray-100 dark:border-dark-100">
                    <div className={`font-body text-xs font-semibold mb-0.5 ${statColors[key]}`}>{key}</div>
                    <div className="font-heading font-bold text-sm text-gray-900 dark:text-gray-100">{val}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="bg-white dark:bg-dark-300 rounded-2xl border border-gray-100 dark:border-dark-100 p-6">
        <h2 className="font-heading text-lg font-bold text-gray-900 dark:text-gray-100 mb-4">
          ...
          <span className="ml-2 font-body font-normal text-sm text-gray-400 dark:text-gray-500">({userDigimon.length})</span>
        </h2>

        {userDigimon.length === 0 ? (
          <div className="text-center py-10 font-body text-gray-400 dark:text-gray-500">No Digimon yet</div>
        ) : (
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3">
            {userDigimon.map(digimon => (
              ...
              >
                ...
                <div className="font-body text-xs text-gray-400 dark:text-gray-500">Lv. {digimon.current_level}</div>
              </div>
            ))}
          </div>
        )}
