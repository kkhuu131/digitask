import { getDigidexProgress } from './digidexProgress';
...
export function countDiscoveries(rows: { user_id: string; digimon_id: number }[]) {
  ...
  return new Map(
    [...speciesByUser].map(([id, species]) => [id, getDigidexProgress(species).count])
  );
}
