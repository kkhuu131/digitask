  ...
  ({
    ...
    if (data.zoomLevel < 0.4) {
      return (
        ...
        >
          <Handle type="target" position={Position.Left} />
          <p className="text-center text-xs truncate">{data.isDiscovered ? data.name : '???'}</p>
          <Handle type="source" position={Position.Right} />
        </div>
      );
    }
...
const DigimonEvolutionGraph: React.FC = () => {
  ...
  const filteredEdges = useMemo(() => {
    ...
    return evolutionPaths
      .filter(
        (p) => filteredDigimonIds.has(p.from_digimon_id) && filteredDigimonIds.has(p.to_digimon_id)
      )
      .map((p) => {
        // Only highlight edges connected to the selected Digimon
        const isSelected =
          selectedDigimon?.id === p.from_digimon_id || selectedDigimon?.id === p.to_digimon_id;
  ...
  useEffect(() => {
    ...
    const digimonByStage = uniqueStages.reduce(
      (acc, stage) => {
    ...
    const maxDigimonInColumn = Math.min(
      MAX_DIGIMON_PER_COLUMN,
      Math.max(
        ...Object.values(digimonByStage).map((group) =>
    ...
    const newNodes: Node[] = filteredDigimon.map((d) => {
      ...
      const isHighlighted =
        ...
        evolutionPaths.some(
          (p) =>
            selectedDigimon?.id &&
            ((p.from_digimon_id === selectedDigimon.id && p.to_digimon_id === d.id) ||
    ...
    if (filteredDigimon.length !== prevSearchLength) {
      ...
      setTimeout(() => {
        if (
          ...
          filteredDigimon.length < digimon.length
        ) {
  ...
  const handleEvolutionClick = useCallback((digimonToSelect: Digimon) => {
    ...
    setTimeout(() => {
      if (reactFlowInstance.current) {
        ...
        if (node) {
          reactFlowInstance.current.setCenter(node.position.x + 150, node.position.y + 50, {
            zoom: 1,
            duration: 800,
          });
        }
