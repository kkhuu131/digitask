        ...
        <div className="bg-white dark:bg-dark-300 rounded-lg p-6 max-w-md w-full">
          ...
          <div className="mb-4">
            <p className="text-sm text-gray-700 dark:text-gray-300">
              This evolution requires a DNA fusion with{' '}
              <span className="font-bold text-yellow-600 dark:text-yellow-400">
                {requiredDigimonData.name}
              </span>
              .
              {candidateDigimon.length > 0
                ? 'Select which of your ' +
                  requiredDigimonData.name +
                  ' you want to use for this fusion:'
                : "You don't have any " +
                  requiredDigimonData.name +
                  ' to use for fusion. You need to obtain one first.'}
            </p>
            {candidateDigimon.length > 0 && (
          ...
          {candidateDigimon.length > 0 ? (
            ...
            <div className="mt-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg">
              <p className="text-center text-yellow-700 dark:text-yellow-300">
                You need to obtain a {requiredDigimonData.name} first before you can perform this
                DNA fusion.
              </p>
            </div>
          )}
