...
const UserTitles: React.FC<UserTitlesProps> = ({ titles: initialTitles, isOwnProfile }) => {
  ...
  const allTitles =
    titles && titles.length > 0
      ? titles.sort(
          (a, b) =>
            getTierPriority(b.title?.tier || 'bronze') - getTierPriority(a.title?.tier || 'bronze')
        )
      : [];
  ...
  const toggleTitleDisplay = (titleId: number, currentlyDisplayed: boolean) => {
    ...
    const updatedTitles = titles.map((title) =>
      title.id === titleId ? { ...title, is_displayed: !currentlyDisplayed } : title
    );
    ...
    if (!currentlyDisplayed && displayedTitles.length >= 3) {
      ...
      const oldestDisplayedTitle = displayedTitles.sort(
      ...
      const finalUpdatedTitles = updatedTitles.map((title) =>
        title.id === oldestDisplayedTitle.id ? { ...title, is_displayed: false } : title
      );
      ...
      {!isEditMode ? (
            ...
            displayedTitles.map((userTitle) => {
              ...
              return (
                ...
                >
                  {/* Tier badge */}
                  <div
                    className={`absolute -top-2 -right-2 ${style.badge} text-xs px-2 py-0.5 rounded-full capitalize`}
                  >
                    {tier}
                  </div>

                  <div className={`font-medium ${style.text}`}>{userTitle.title?.name}</div>
                  <div className="text-xs text-gray-600 mt-1">{userTitle.title?.description}</div>
        ...
        <div className="space-y-4">
          <p className="text-sm text-gray-500 mb-2">
            Select up to 3 titles to display on your profile:
          </p>
                ...
                >
                  <div className="flex-1">
                    <div className="flex items-center">
                      <div
                        className={`font-medium ${userTitle.is_displayed ? style.text : 'text-gray-700'}`}
                      >
                        {userTitle.title?.name}
                      </div>
                      <span
                        className={`ml-2 text-xs ${style.badge} px-2 py-0.5 rounded-full capitalize`}
                      >
                        {tier}
                      </span>
                    </div>
                    <div className="text-xs text-gray-600">{userTitle.title?.description}</div>
