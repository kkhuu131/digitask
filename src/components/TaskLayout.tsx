  ...
  const filteredTasks = useMemo(() => {
    let filtered = tasks.filter((task) => {
      ...
      if (activeTab === 'today') {
        return (
          !task.is_completed &&
          (isTaskDueToday(task) || isRecurringTaskForToday(task) || task.is_daily)
        );
    ...
    if (filters.categories.length > 0) {
      filtered = filtered.filter(
        (task) => task.category && filters.categories.includes(task.category as StatCategory)
      );
    }

    if (filters.difficulties.length > 0) {
      filtered = filtered.filter(
        (task) => task.difficulty && filters.difficulties.includes(task.difficulty)
      );
    }
    ...
    if (filters.types.length > 0) {
      filtered = filtered.filter((task) => {
        ...
        if (
          ...
          return true;
        if (
          filters.types.includes('one-time') &&
          !task.is_daily &&
  ...
  const tabs = [
    {
      id: 'today',
      label: 'Today',
      count: (todayGroups.overdue?.length || 0) + (todayGroups.dueToday?.length || 0),
    },
          ...
          {activeTab === 'upcoming' && (
            <div className="space-y-4">
              {Object.entries(upcomingGroups).map(([dateKey, tasks]) => (
                <div
                  key={dateKey}
                  className="bg-white dark:bg-dark-300 border border-gray-200 dark:border-dark-200 rounded-lg"
                >
