...
const InteractiveBattle: React.FC<InteractiveBattleProps> = ({
  onBattleComplete,
  userDigimon = [],
  battleOption,
  showRewards = true,
  ...
  const [showBattleResult, setShowBattleResult] = useState<{
    winner: 'user' | 'opponent';
    show: boolean;
    bitsReward?: number;
  } | null>(null);
  ...
  const getUpcomingTurnOrder = React.useCallback(() => {
    ...
    const alive = [...currentBattle.userTeam, ...currentBattle.opponentTeam].filter(
      (d) => d.isAlive
    );
    ...
    const startIdx = Math.max(
      0,
      order.findIndex((d) => d.id === currentBattle.currentAttacker)
    );
    ...
    if (battleStatus.isBattleComplete && currentBattle && !showBattleResult) {
      ...
      if (showRewards && !skipResultScreen) {
        if (customRewards) {
          ...
          bitsReward = calculateBitsReward(
            battleOption.difficulty,
            currentBattle.winner === 'user'
          );
        }
      }
  ...
  useEffect(() => {
    if (currentBattle && currentBattle.turnHistory.length > 0) {
      ...
      if (latestTurn !== lastTurn) {
        ...
        const attacker = [...currentBattle.userTeam, ...currentBattle.opponentTeam].find(
          (d) => d.id === latestTurn.attacker.id
        );
        const target = [...currentBattle.userTeam, ...currentBattle.opponentTeam].find(
          (d) => d.id === latestTurn.target.id
        );
        ...
        >
          <div className="flex items-center gap-2 w-full">
            <TypeAttributeIcon
              ...
              size="sm"
            />
            <h3 className="font-bold text-sm truncate max-w-[120px] dark:text-gray-100 text-gray-900">
              {digimon.name}
            </h3>
          </div>
          ...
          <div className="mt-2">
            <div className="flex items-center gap-2">
              <span className="text-[10px] px-1.5 py-0.5 rounded bg-black/70 text-white dark:bg-white/20 dark:text-white whitespace-nowrap">
          ...
          {isCurrentAttacker && (
            <div
              className={`absolute -top-2 ${isUserTeam ? '-right-2' : '-left-2'} bg-yellow-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold`}
            >
        ...
        >
          ...
          {isLastTurnTarget && (
            <motion.div
              ...
              animate={{
                backgroundColor: ['rgba(255,0,0,0.0)', 'rgba(255,0,0,0.25)', 'rgba(255,0,0,0.0)'],
              }}
              transition={{ duration: 0.4 }}
            />
          )}
          ...
          {isSelected && currentAttacker && (
            ...
            >
              {(() => {
                ...
                return (
                  <div
                    className={`px-2 py-1 rounded-md text-xs font-bold ${display.bgColor} ${display.color}`}
                  >
                    {display.text}
                  </div>
                );
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4 relative">
      ...
      <div className="text-center mb-4">
        <h2 className="text-2xl font-heading font-semibold mb-2 dark:text-gray-100 text-gray-900">
          Battle
        </h2>
      </div>
      ...
      <div className="flex items-start gap-4 mb-6">
        ...
        >
          ...
          <div className="absolute inset-0 bg-green-900 bg-opacity-90">
            ...
            <div
              ...
              style={{
                backgroundImage:
                  'linear-gradient(to right, rgba(144, 238, 144, 0.2) 1px, transparent 1px), linear-gradient(to bottom, rgba(144, 238, 144, 0.2) 1px, transparent 1px)',
        ...
        <div className="w-28 sm:w-32">
          <div className="bg-gray-300/40 dark:bg-gray-800/40 backdrop-blur-sm rounded-lg border border-gray-400/20 dark:border-gray-600/20 p-2">
            <p className="text-xs font-semibold text-gray-800 dark:text-gray-200 mb-2 text-center">
              {battleStatus.isPlayerTurn ? 'Player Turn' : 'Enemy Turn'}
            </p>
            <div className="flex flex-col gap-2">
              {getUpcomingTurnOrder().map((d, idx) => (
                <div
                  ...
                  className={`flex items-center justify-center h-12 rounded-md bg-white/10 dark:bg-white/5 ${
                    d.isOnUserTeam
                      ? 'border-2 border-blue-500/60 dark:border-blue-400/60'
                      : 'border-2 border-red-500/60 dark:border-red-400/60'
              ...
              {getUpcomingTurnOrder().length === 0 && (
                <div className="text-[11px] text-gray-600 dark:text-gray-400 text-center py-3">
                  No turns
                </div>
              )}
            </div>
      ...
      {battleStatus.isPlayerTurn && !battleStatus.isBattleComplete && (
        <div className="text-center h-32 flex flex-col justify-center">
          <div className="mb-4">
            <p className="text-lg font-semibold text-blue-600">{currentAttacker?.name}'s Turn</p>
            <p className="text-sm text-gray-600">Select a target to attack</p>
          </div>
      ...
      {!battleStatus.isPlayerTurn && !battleStatus.isBattleComplete && (
        <div className="text-center h-32 flex flex-col justify-center">
          <div className="animate-pulse">
            <p className="text-lg font-semibold text-red-600">Opponent's Turn</p>
            <p className="text-sm text-gray-600">{currentAttacker?.name} is thinking...</p>
          </div>
        </div>
      )}
