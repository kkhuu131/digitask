...
const CleanTaskList: React.FC<CleanTaskListProps> = ({
  showCompleted = false,
    ...
    return allTasks.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case 'type':
          ...
          return (
            typeOrder[aType as keyof typeof typeOrder] - typeOrder[bType as keyof typeof typeOrder]
          );
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5">
                      {task.category && CATEGORY_COLORS[task.category] && (
                        <span
                          className={`w-2 h-2 rounded-full flex-shrink-0 ${CATEGORY_COLORS[task.category]}`}
                        />
                      )}
                      <p
                        className={`text-base ${
                    ...
                    <div className="flex items-center flex-wrap gap-1.5 mt-1.5">
                      ...
                      {!task.is_completed &&
                        task.priority === 'high' &&
                        !groupedTasks.overdue.includes(task) && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 font-body font-semibold">
                            High
                          </span>
                        )}
                      ...
                      {/* Task type */}
                      {task.is_daily && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 font-body font-semibold uppercase tracking-wide">
                          Daily
                        </span>
                      )}
                      {task.recurring_days && task.recurring_days.length > 0 && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 font-body font-semibold uppercase tracking-wide">
                          Recurring
                        </span>
                      )}
                      ...
                      {formatDueDate(task.due_date) && (
                        <span
                          className={`text-xs font-body font-medium ${getTaskStatusColor(task)}`}
                        >
                          {formatDueDate(task.due_date)}
                        </span>
                      )}
