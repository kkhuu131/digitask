-- Remove daily stat cap system
-- This migration removes the daily stat cap limitation that was preventing users from gaining unlimited stats per day

-- First, update the complete_task_all_triggers function to remove daily stat cap logic
...
BEGIN
  ...
  -- STEP 3: Award experience points and increase happiness
  BEGIN
    ...
    -- Calculate base exp points and stat boost based on task type
    ...
    CASE v_task_record.difficulty
      WHEN 'easy' THEN 
        v_difficulty_exp_multiplier := 0.7;
        v_difficulty_stat_multiplier := 1.0;
      WHEN 'medium' THEN 
        v_difficulty_exp_multiplier := 1.0;
        v_difficulty_stat_multiplier := 1.0;
      WHEN 'hard' THEN 
        v_difficulty_exp_multiplier := 1.5;
        v_difficulty_stat_multiplier := 2.0;
      ELSE -- Default to medium if null or invalid
        v_difficulty_exp_multiplier := 1.0;
        v_difficulty_stat_multiplier := 1.0;
    END CASE;

    -- Apply priority multiplier to EXP only
    CASE v_task_record.priority
      WHEN 'low' THEN 
        v_priority_exp_multiplier := 0.8;
      WHEN 'medium' THEN 
        v_priority_exp_multiplier := 1.0;
      WHEN 'high' THEN 
        v_priority_exp_multiplier := 1.3;
      ELSE -- Default to medium if null or invalid
        v_priority_exp_multiplier := 1.0;
    END CASE;

    -- Apply all multipliers
    v_exp_points := ROUND(v_exp_points * v_exp_multiplier * v_difficulty_exp_multiplier * v_priority_exp_multiplier);
    v_boost_value := ROUND(v_boost_value * v_difficulty_stat_multiplier);
  ...
  -- STEP 4: Handle stat allocation (NO DAILY CAP)
  BEGIN
    -- Get the user's profile with saved_stats
    SELECT saved_stats
    INTO v_saved_stats
    FROM profiles
    WHERE id = p_user_id;
    ...
    -- Process stat points if task has a category (NO DAILY CAP CHECK)
    IF v_task_category IS NOT NULL THEN
      -- Use the full boost value (no daily cap limitation)
...
COMMENT ON FUNCTION complete_task_all_triggers IS 'Completes a task and handles all operations in separate transactions, increases happiness by 10 (max 100), and avoids ALL trigger conflicts. NO DAILY STAT CAP.';

-- Remove the daily_stat_gains column from profiles table
ALTER TABLE profiles DROP COLUMN IF EXISTS daily_stat_gains;

-- Remove the last_stat_reset column from profiles table (no longer needed)
ALTER TABLE profiles DROP COLUMN IF EXISTS last_stat_reset;

-- Remove the daily_stat_gains column from user_digimon table (if it exists)
ALTER TABLE user_digimon DROP COLUMN IF EXISTS daily_stat_gains;

-- Remove the last_stat_reset column from user_digimon table (if it exists)
ALTER TABLE user_digimon DROP COLUMN IF EXISTS last_stat_reset;
